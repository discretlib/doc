...
- **name**: le nom de l'autorisation, par exemple: auteurs, lecteurs, etc.
- **rights**: les droits d'accès au niveau des entité: quelle entité est acceptée dans cette Room et quelles sont les autorisations d'écriture pour cette autorisation,
- **users**:  les utilisateurs autorisés,
- **user_admin**: qui peut gérer des utilisateurs de cette autorisation.
...
- **mdate**: le champ système **mdate** définit la date de début de validité de cette autorisation.
...
- **mutate_self**: indique si vous pouvez insérer de tuples de cette entité, et modifier les tuples que vous avez inséré ,
...
- il n'est pas possible de supprimer, les entités d'une *Room*: par exemple, une fois qu'une **authorisation** a été créée, elle ne peut plus être supprimée. 
...
Vous noterez que:
- nous n'avons pas défini de droits sur l'entité **Article** pour les lecteurs, car toute personne ayant accès à la *Room* a un accès en lecture seule sur toutes les entités. 
- aucune des deux autorisations n'a défini de champ **user_admin**, cela signifie que seul les **admin** (ici *$admin_user*) de la *Room* peuvent ajouter ou désactiver des utilisateurs
